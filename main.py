...
from flask import Flask, make_response, render_template, request, session, flash, redirect, url_for
...
def give():
    ...
    if request.method == "POST":
        ...
        with app.app_context():
            ...
            def_pfp = "static/Admin/default.jpg"
...
def pictures():
    ...
    if request.method == 'POST':
        ...
        os.makedirs(f"static/users/{u}", exist_ok=True)
        if pict:
            pict.save(os.path.join(f"static/users/{u}/", pict.filename))
            path = os.path.join(f"static/users/{u}/", pict.filename)
...
def edit():
    ...
    if request.method == 'POST':
        ...
        os.makedirs(f"static/users/{u}/profile", exist_ok=True)
        if pfp:
            pfp.save(os.path.join(f"static/users/{u}/profile", f"{u}.jpg"))
            path = os.path.join(f"static/users/{u}/profile", f"{u}.jpg")
        user = db.session.execute(db.select(Store).filter_by(name=u)).scalar()
        user.about = ab
        user.profile = path
        db.session.commit()
